...
const ProjectPerformanceReport: React.FC<OwnProps> = () => {
  const { classes } = useStyles();
  const [startDate, setStartDate] = React.useState(new Date());
  const [endDate, setEndDate] = React.useState(new Date());
  const [records, _setRecords] = React.useState<IProject[]>([]);
  ...
  const handleExportPDF = async () => {
    ...
    // const opt = {
    //   margin: 0.2,
    //   filename: `Forklift-Breakdown-Report-${stDate.getFullYear()}-${
    //     stDate.getMonth() + 1
    //   }-${stDate.getDate()}.pdf`,
